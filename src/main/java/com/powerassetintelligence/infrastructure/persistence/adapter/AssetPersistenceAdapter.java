...
public class AssetPersistenceAdapter implements AssetRepositoryPort {
    ...
    public Asset save(Asset asset) {
        com.powerassetintelligence.infrastructure.persistence.entity.AssetEntity entity = repository.findById(asset.getId())
                .orElseGet(() -> new com.powerassetintelligence.infrastructure.persistence.entity.AssetEntity(
                        asset.getId(),
                        asset.getType(),
                        asset.getName(),
        ...
        entity.setManufacturer(asset.getManufacturer());
        entity.setCriticality(asset.getCriticality());
        entity.setExpectedServiceLifeYears(asset.getExpectedServiceLifeYears());
    ...
    public Optional<Asset> findById(UUID assetId) {
        return repository.findById(assetId).map(PersistenceMapper::toDomain);
    }

    @Override
    public PageResult<Asset> search(AssetType type, AssetStatus status, AssetCriticality criticality, String location, PageRequest pageRequest) {
        var springPageable = PersistenceMapper.toSpringPageable(pageRequest);
        Page<com.powerassetintelligence.infrastructure.persistence.entity.AssetEntity> result =
                repository.search(type, status, criticality, location, springPageable);
